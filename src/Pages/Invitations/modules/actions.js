...
export const respondToTranslationInvitation = (organizationId, articleId, status, inviteToken, email) => (dispatch) => {
    ...
    .then((res) => {
        const { user, token, speakerNumber } = res.body;
...
export const respondToOrganizationInvitation = (organizationId, status, inviteToken, email) => (dispatch) => {
    dispatch(setLoading(true))
    requestAgent.post(Api.invitations.respondToOrganizationInvitation(organizationId), { inviteToken, status, email })
    .then((res) => {
