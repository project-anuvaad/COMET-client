...
function Separator() {
  return (
    <span style={{ display: "inline-block", margin: "0 10px", color: "gray" }}>
      |
    </span>
  );
}
...
class Review extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (activeTab) {
      switch (activeTab) {
        case "transcribe":
          this.setState({ activeTab: "transcribe" });
          break;
        case "proofread":
          this.setState({ activeTab: "proofread" });
          break;
        case "completed":
    ...
    this.props.setCurrentPageNumber(1);
    this.props.setVideoStatusFilter(
      videoStatusMap[activeTab || TABS.CUT_VIDEO]
    );
    this.props.fetchVideos();
    this.props.fetchVideosCount(this.props.organization._id);
    this.videoUploadedSub = websockets.subscribeToEvent(
      websockets.websocketsEvents.VIDEO_UPLOADED,
      (data) => {
        this.props.fetchVideos();
        this.props.fetchVideosCount(this.props.organization._id);
      }
    ...
    this.videoTranscribedSub = websockets.subscribeToEvent(
      websockets.websocketsEvents.VIDEO_TRANSCRIBED,
      this.onVideoTranscribed
    );
    this.videoDoneSub = websockets.subscribeToEvent(
      websockets.websocketsEvents.VIDEO_DONE,
  ...
  onVideoTranscribed = (video) => {
    if (this.props.videos.map((video) => video._id).indexOf(video._id) !== -1) {
      ...
      NotificationService.info(
        `${video.title} available in the "Proofread" stage`
      );
    }
  };
  ...
  onVideoCompleted = (video) => {
    ...
    NotificationService.success(
      `The video "${video.title}" has been converted successfully!`
    );
  };
  ...
  renderEditVideoModal = () => (
    <EditVideoModal
      ...
      onReset={() =>
        this.setState({ tmpEditVideo: { ...this.state.selectedVideo } })
      }
      onChange={(changes) => {
      ...
      onSave={() =>
        this.onSaveEditedVideo(
          this.state.selectedVideo,
          this.state.tmpEditVideo
        )
      }
    />
  );
  ...
  renderSelectCutterModal = () => (
    ...
    >
      ...
      <Modal.Content>
        <p>
          Would you like to do that yourself or let Videowiki's team do it for
          you?
        </p>
      </Modal.Content>
      <Modal.Actions>
        ...
        <Button circular primary onClick={() => this.onSkipTranscribe("self")}>
          I'll do it myself
        </Button>
      </Modal.Actions>
    </Modal>
    ...
    >
      <Modal.Header>
        ...
        <Button
          ...
          style={{ position: "absolute", right: "1rem", top: "1rem" }}
          onClick={() =>
            this.setState({ isNotifyWithWhatsappModalOpen: false })
          }
        />
      </Modal.Header>
      <Modal.Content>
        <p>We'll send you an email once the video is ready for proofreading</p>
        <p>Would you like to get a notification through WhatsApp too?</p>
      </Modal.Content>
      <Modal.Actions>
        <Button
          circular
        ...
        {this.props.selectedVideo && (
          <a
            href={getWhatsappNotifyOnProofreadingReady(
              this.props.selectedVideo._id
            )}
            target="_blank"
  ...
  _renderDeleteVideoModal = () => (
    <Modal
      ...
      size="tiny"
      onClose={() =>
        this.setState({ deleteVideoModalVisible: false, deletedVideo: null })
      }
    >
      <Modal.Header>
        CONFIRM DELETE
      ...
      <Modal.Content>
        <div>
          ...
          <small>(All associated content/articles will be deleted)</small>
        </div>
        <Grid>
          {this.props.videos
            .filter((v) => v.selected)
            .map((v) => (
              ...
              >
                ...
                <Grid.Column width={12} style={{ alignItems: "center" }}>
                  ...
                  {v.duration && <small>{formatTime(v.duration * 1000)}</small>}
                </Grid.Column>
              </Grid.Row>
            ))}
        </Grid>
      ...
      <Modal.Actions>
        ...
        <Button circular primary onClick={this.deleteSelectedVideos}>
          Yes, Delete
        </Button>
      </Modal.Actions>
    </Modal>
  ...
  renderAssignUsers = () => (
    <AssignReviewUsers
      ...
      value={
        this.state.selectedVideo
          ? this.state.selectedVideo.reviewers.map((r) => r._id)
          : []
      }
      users={getUsersByRoles(
      ...
      onResendEmail={(userId) =>
        this.onResendEmail("reviewer", this.state.selectedVideo._id, userId)
      }
    />
  );
  ...
  renderAssignVerifiers = () => (
    <AssignReviewUsers
      ...
      value={
        this.state.selectedVideo && this.state.selectedVideo.verifiers
          ? this.state.selectedVideo.verifiers.map((r) => r._id)
          : []
      }
      users={getUsersByRoles(
        this.props.organizationUsers,
      ...
      onResendEmail={(userId) =>
        this.onResendEmail("verifier", this.state.selectedVideo._id, userId)
      }
    />
  );
  ...
  renderConfirmReviewModal = () => (
    <Modal open={this.state.confirmReviewModalVisible} size="tiny">
      ...
      <Modal.Content>
        <p>
          Are you sure you want to re-review this video?{" "}
          <small>
            <strong>( All current translations will be archived )</strong>
          </small>
        </p>
      </Modal.Content>
      <Modal.Actions>
      ...
      renderedComp =
        this.props.videos && this.props.videos.length === 0 ? (
          ...
          this.props.videos.map((video) => {
            ...
            const animate =
              !loading &&
              this.props.videosCounts &&
              this.props.videosCounts.total === 1 &&
              this.props.videosCounts.cutting === 1;
            const whatsappIconTarget = generateWhatsappTranscribeLink(
              video._id
            );
        ...
        this.props.videos && this.props.videos.length === 0 ? (
          ...
          this.props.videos.map((video) => {
            ...
            const animate =
              ...
              this.props.videosCounts &&
              this.props.videosCounts.total === 1 &&
              this.props.videosCounts.proofread === 1;
  ...
  _renderTabContent = () => {
    return (
      <LoaderComponent active={this.props.videosLoading}>
        ...
        <Grid.Row>{this.renderVideosCards()}</Grid.Row>
      </LoaderComponent>
    );
  };
  ...
  render() {
    const SUBTABS = [
      {
        title: `Break video into slides (${formatCount(
          this.props.videosCounts.cutting
        )})`,
        value: TABS.CUT_VIDEO,
      },
      {
        title: `Proofread (${formatCount(this.props.videosCounts.proofread)})`,
        value: TABS.PROOFREAD,
      },
      {
        title: `Completed (${formatCount(this.props.videosCounts.completed)})`,
    ...
    const allSelected =
      ...
      this.props.selectedCount === this.props.videos.length;
                  ...
                  >
                    <label>
                      <input
                        ...
                        checked={allSelected}
                        onClick={() =>
                          this.props.setAllVideoSelected(!allSelected)
                        }
                      />
                      Select all videos
                    </label>
                        ...
                        <Dropdown text="Transcribe">
                          <Dropdown.Menu>
                            <Dropdown.Item
                              text="All videos"
                              onClick={() =>
                                this.onTranscribeVideo({ _id: "all" })
                              }
                    ...
                    {this.props.selectedCount > 0 && (
                      <React.Fragment>
                        ...
                        <span
                          ...
                          onClick={() =>
                            this.setState({ deleteVideoModalVisible: true })
                          }
                        >
                          <Icon name="trash alternate outline" color="blue" />{" "}
                    ...
                    {this.props.selectedCount > 0 && (
                      <React.Fragment>
                        ...
                        <span
                          ...
                          onClick={() =>
                            this.setState({
                              assignUsersToMultipleVideosModalOpen: true,
                            })
                          }
                        >
                          <Icon name="add" size="small" color="blue" /> Add
                          Transcribers To Selected Videos
                        </span>
                      </React.Fragment>
                    )}
                    {this.props.selectedCount > 0 && (
                      <React.Fragment>
                        ...
                        <span
                          ...
                          onClick={() =>
                            this.setState({
                              assignVerifiersToMultipleVideosModalOpen: true,
                            })
                          }
                        >
                          <Icon name="add" size="small" color="blue" /> Add
                          Verifiers To Selected Videos
                        </span>
                      </React.Fragment>
                    )}
                  </div>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  updateVideo: (videoId, changes) =>
    dispatch(videoActions.updateVideo(videoId, changes)),
  deleteVideo: (videoId) => dispatch(videoActions.deleteVideo(videoId)),
  setLanguageFilter: (langCode) =>
    dispatch(videoActions.setLanguageFilter(langCode)),
  ...
  setVideoStatusFilter: (filter) =>
    dispatch(videoActions.setVideoStatusFilter(filter)),
  ...
  skipTranscribe: (video, cuttingBy) =>
    dispatch(videoActions.skipTranscribe(video, cuttingBy)),
  updateVideoReviewers: (videoId, users) =>
    dispatch(videoActions.updateVideoReviewers(videoId, users)),
  updateVideosReviewers: (users) =>
    dispatch(videoActions.updateVideosReviewers(users)),
  resendEmailToVideoReviewer: (videoId, userId) =>
    dispatch(videoActions.resendEmailToVideoReviewer(videoId, userId)),
  updateVideoVerifiers: (videoId, users) =>
    dispatch(videoActions.updateVideoVerifiers(videoId, users)),
  updateVideosVerifiers: (users) =>
    dispatch(videoActions.updateVideosVerifiers(users)),
  resendEmailToVideoVerifier: (videoId, userId) =>
    dispatch(videoActions.resendEmailToVideoVerifier(videoId, userId)),
  setVideoSelected: (videoId, selected) =>
    dispatch(videoActions.setVideoSelected(videoId, selected)),
  setAllVideoSelected: (selected) =>
    dispatch(videoActions.setAllVideoSelected(selected)),
  trancibeSelectedVideos: () =>
    dispatch(videoActions.transcribeSelectedVideos()),
